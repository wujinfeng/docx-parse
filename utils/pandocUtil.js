...
/**
 * 转word, pandoc版本2.7, 1.9
 * @returns {Promise<any>}
 */
let pandocV27TransDocx = function (templateFile, inputFile, outFile) {
    return new Promise((resolve, reject) => {
        let cmd = 'pandoc -f html+smart -t docx+smart --reference-doc ' + templateFile + ' -o ' + outFile + ' ' + inputFile;
        debug('pandoc2.7命令：', cmd);
...
let strategies = {
    ...
    '2.7': async function (templateFile, inputFile, outFile) {
        return await pandocV27TransDocx(templateFile, inputFile, outFile);
    }
};
